...
export default function Messages() {
  ...
  const scrollMarkRef = useRef(null);

  const scrollToBottom = () => {
    console.log(scrollMarkRef);
    scrollMarkRef.current.scrollIntoView({ behavior: 'smooth' });
  };

  useEffect(() => {
    if (scrollMarkRef.current) {
      scrollToBottom();
    }
  }, [messages]);
  //#endregion
      ...
      <Fragment key={message.uuid}>
        {index === 0 && (
          <div className="invisible">
            <hr className="m-0" ref={scrollMarkRef} />
          </div>
        )}
  ...
  return (
    <Col xs={10} md={8}>
      <div className="messages-box">
        <div className="d-flex flex-column-reverse">{selectedChatMarkup}</div>
      </div>
      <div>
